...
### Streamlined Customer Checkout Flow (April 2026)
- New `client/src/components/quick-checkout-dialog.tsx` — popup with two paths: "Login + earn points" or "Quick checkout (name + phone only)". The quick path sets `qahwa-guest-mode` + `qahwa-guest-info` in localStorage and navigates to `/checkout`.
- `cart-modal.tsx` checkout button now: (1) auto-fills delivery info and skips `/delivery` when only one branch is active, (2) opens `QuickCheckoutDialog` for unauthenticated users instead of redirecting to `/auth`.
- `delivery-selection.tsx` also auto-skips itself if branches.length === 1 (defense in depth).
- Apple Pay no longer uses native `ApplePaySession` on the customer checkout page. The custom Apple Pay button was removed; `apple_pay` now appears in `PaymentMethods` and routes through Geidea HPP via `isOnlinePaymentMethod`.
- Product images in `menu.tsx` and `menu-layouts.tsx` use `loading="lazy"` + `decoding="async"` to speed up first paint on the menu page.
