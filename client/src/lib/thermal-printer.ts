...
export async function buildShiftReportCanvas(opts: ShiftReportOpts): Promise<HTMLCanvasElement> {
  ...
  // ── FOOTER ────────────────────────────────────────────────────────────────
  drawDash();
  drawCenter(`CLUNY SYSTEMS — ${new Date().toLocaleString('ar-SA')}`, Math.round(FS * 0.8), false, '#666');
 ...
 */
/**
 * Cloud Print Queue mode — posts the job to the CLUNY CAFE server.
