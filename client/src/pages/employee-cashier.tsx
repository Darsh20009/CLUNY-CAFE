...
export default function EmployeeCashier() {
 ...
 const { data: businessConfig } = useQuery<any>({
  ...
  const createOrderMutation = useMutation({
    mutationFn: async (orderData: any) => {
      ...
      const pmLabels: Record<string, string> = { cash: tc("نقداً","Cash"), pos: tc("شبكة","Network"), "pos-network": tc("شبكة","Network"), "qahwa-card": tc("بطاقة كلوني","CLUNY Card"), "loyalty-card": tc("بطاقة ولاء","Loyalty Card") };
 ...
 paymentMethod === "qahwa-card" || paymentMethod === "loyalty-card" ? tc("بطاقة كلوني","CLUNY Card") :
 tc("شبكة","Network");
   ...
   try {
     ...
     if (order) {
       ...
       const pmLabel = capturedPaymentMethod === "cash" ? tc("نقدي","Cash") :
         capturedPaymentMethod === "qahwa-card" || capturedPaymentMethod === "loyalty-card" ? tc("بطاقة كلوني","CLUNY Card") :
 ...
 <img src={clunyLogoStaff} alt="CLUNY" className="w-full h-full object-contain rounded-2xl" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
 </div>
 <div>
 ...
 <SelectValue />
    ...
    <SelectContent className="bg-gray-50 border-gray-300 text-gray-900">
      ...
      <SelectItem value="qahwa-card">{tc("بطاقة كلوني (ولاء)","CLUNY Card (Loyalty)")}</SelectItem>
    </SelectContent>
 </Select>
