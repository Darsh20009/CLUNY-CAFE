...
export default function OrderTracker({ order, compact = false }: OrderTrackerProps) {
 ...
 const computeRemaining = () => {
   const estimatedMinutes = (order as any).estimatedTime ?? (order as any).estimatedPrepTimeInMinutes;
   const activeStatuses = ['in_progress', 'payment_confirmed', 'pending'];
   if (!estimatedMinutes || !activeStatuses.includes(order.status)) return null;
   const startedAt = (order as any).prepTimeSetAt
     ? new Date((order as any).prepTimeSetAt)
     : new Date((order as any).updatedAt || Date.now());
   const totalSeconds = Number(estimatedMinutes) * 60;
 ...
 }, [order.status, (order as any).estimatedTime, (order as any).estimatedPrepTimeInMinutes]);

 const formatCountdown = (secs: number) => {
   if (secs <= 0) return 'جاهز قريباً!';
 ...
 {/* Estimated Time (if active order) */}
 {(order.status === "payment_confirmed" || order.status === "in_progress" || order.status === "pending") && (
 <motion.div
 initial={{ opacity: 0 }}
 animate={{ opacity: 1 }}
 ...
 {remainingSeconds !== null ? (
   <div className="space-y-1">
     <p className="text-xs text-gray-500">الوقت المتبقي للتحضير</p>
     <p className={`text-lg font-bold ${remainingSeconds === 0 ? 'text-green-400' : 'text-amber-400'}`}>
