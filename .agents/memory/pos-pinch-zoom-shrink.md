...
## Fourth root cause found (same symptom, precisely tied to order checkout): dead code calling html2canvas after every order
User later reported the shrink happens *specifically right after completing a new order* — a genuinely distinct, code-triggered signal, not the generic OS gesture above. Root cause: a receipt-PNG "pre-render" step ran on every single checkout via `html2canvas`, whose result was never actually consumed anywhere in the app (fully dead code — nothing called the getter). `html2canvas` creates a temporary `<iframe>` internally to clone the DOM for rendering; per the iframe rule (see `android-print-no-iframe.md`), ANY iframe appearing in the DOM on Android WebView — even off-screen/invisible ones — forces a viewport recalculation that visibly shrinks the whole app for a moment.
**Why this matters**: when a shrink report gets a specific trigger ("right after X action"), treat it as a new, code-level lead rather than folding it into an already-explained OS-level theory — trace what code actually runs at that trigger point before concluding it's unfixable.
**Fix applied**: deleted the unused `receipt-png-cache.ts` / `render-receipt-preview.tsx` files and removed the `preRenderReceiptPng(...)` call from the checkout handler in `pos-system.tsx` entirely. No functionality was lost since the PNG was never read.
