...
async function executeAction(action: any, event: string, data: any, tenantId?: string): Promise<void> {
  ...
  if (type === "webhook") {
    ...
    await fetch(config.url, {
      method: "POST",
      headers: { "Content-Type": "application/json", "User-Agent": "CLUNY-Automation/1.0" },
      body,
      signal: ctrl.signal,
    ...
    if (!config.to) return;
    const subject = interpolate(config.subject || "CLUNY: {{event}}", { event, ...data });
    ...
    const text = interpolate(config.message || "CLUNY: {{event}}", { event, ...data });
...
export const INTEGRATION_CATALOG = [
  ...
  { type: "zapier", category: "automation", nameAr: "Zapier", nameEn: "Zapier", icon: "⚡", fields: ["webhookUrl"], description: "ربط CLUNY بـ 5000+ تطبيق عبر Zapier" },
  ...
  // Generic
  { type: "generic_webhook", category: "messaging", nameAr: "Webhook مخصّص", nameEn: "Generic Webhook", icon: "🔗", fields: ["url", "secret"], description: "أرسل أحداث CLUNY لأي نظام خارجي" },
];
